    ...
    try:
        ...
        if not three_d_result:
            ...
            try:
                ...
                # Convert to RGB and resize to reasonable size
                ...
                # Resize if too large (many APIs have size limits)
                max_size = 1024
                ...
                logging.info(f"Processed image: JPEG, {len(jpeg_data)} bytes, size: {image.size}")
                
            except Exception as img_error:
                logging.error(f"Error processing image: {img_error}")
                jpeg_data = image_data

            # Try different formats for the 3D API
            formats_to_try = [
                # Most common format for APIs
                f"data:image/jpeg;base64,{base64.b64encode(jpeg_data).decode('utf-8')}",
                # Alternative formats
                f"data:image/png;base64,{base64.b64encode(image_data).decode('utf-8')}",
                # Just base64 without data URI
                base64.b64encode(jpeg_data).decode('utf-8'),
                base64.b64encode(image_data).decode('utf-8'),
            ]

            for i, image_format in enumerate(formats_to_try):
                try:
                    format_name = ["JPEG data URI", "PNG data URI", "JPEG base64", "PNG base64"][i]
                    logging.info(f"Attempt {i+2}: {format_name} (length: {len(image_format)})")
                    
                    three_d_payload = {"input_image": image_format}
                    ...
                    if three_d_result:
                        logging.info(f"Success with format: {format_name}!")
                        break
                        
                except Exception as format_error:
                    logging.warning(f"Format {format_name} failed: {str(format_error)}")
                    if i == len(formats_to_try) - 1:  # Last attempt
                        raise format_error
                    continue
