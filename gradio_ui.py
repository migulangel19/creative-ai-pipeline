...
{expanded_prompt if expanded_prompt else 'Not available - check details.txt file'}"""
        ...
        # Find GLB and create info - ONLY from current generation
        glb_info = "No 3D model generated in this session"
        glb_file = None
        
        # Only look for GLB files created in the last 30 seconds (current generation)
        current_time = time.time()
        glb_path = find_latest_generated_glb()
        
        if glb_path:
            try:
                file_mod_time = os.path.getmtime(glb_path)
                # Only show if file was created in the last 30 seconds
                if (current_time - file_mod_time) <= 30:
                    file_size = os.path.getsize(glb_path)
                    mod_time = datetime.fromtimestamp(file_mod_time)
...
• Shift + drag to pan"""
                    ...
                    glb_file = glb_path
                    logger.info(f"Loaded fresh GLB: {glb_path}")
                else:
                    glb_info = f"""❌ 3D MODEL GENERATION FAILED

The system attempted to generate a 3D model but failed.
This could be due to:
• Complex prompt requiring image-to-3D conversion
• Network connectivity issues
• API limitations

TIP: Try a simpler, more direct 3D object description like:
"a red cube", "a wooden chair", "a simple car"

Last file found: {os.path.basename(glb_path)} (too old - {int(current_time - file_mod_time)}s ago)"""
                    
            except Exception as e:
                logger.error(f"Error processing GLB: {e}")
                glb_info = f"""❌ 3D MODEL ERROR

Error processing 3D model file: {str(e)}

Please try again with a different prompt."""
        else:
            glb_info = f"""❌ NO 3D MODEL GENERATED

The generation completed but no 3D model was created.
This could be because:
• The prompt was more suitable for 2D images
• 3D generation failed or timed out
• The model couldn't interpret the 3D request

TIP: For 3D models, use clear object descriptions like:
"create a 3D model of a chair"
"generate a 3D car"
"make a 3D house"

Try again with a more specific 3D-focused prompt."""
